...
function generateInitialSchedule(date, time) {
  ...
  for (let first = 0; first < teams.length; first += 1) {
    for (let second = first + 1; second < teams.length; second += 1) {
      pairs.push([teams[first].id, teams[second].id]);
    }
  }
...
$('maintenanceAdminButton').addEventListener('click', openAdmin);
...
$('settingsForm').addEventListener('submit', (event) => {
  ...
  settings = {
    ...
    started: $('settingsStarted').checked,
    finished: $('settingsFinished').checked,
  };
  save();
  renderAll();
  showToast('تم حفظ إعدادات البطولة');
});

$('registrationForm').addEventListener('submit', async (event) => {
  event.preventDefault();
  if (settings.started || settings.registration !== 'open') {
    syncRegistrationState();
    showToast('التسجيل مغلق حاليًا');
    return;
  }
  const team = {
    id: makeId(),
    ...
    players: [
      { name: $('registrationCaptainName').value.trim(), id: String($('registrationCaptainId').value).trim() },
      { name: $('registrationPlayerName').value.trim(), id: String($('registrationPlayerId').value).trim() },
      ...($('registrationSubstituteName').value.trim() || $('registrationSubstituteId').value.trim()
        ? [{ name: $('registrationSubstituteName').value.trim(), id: String($('registrationSubstituteId').value).trim() }]
        : []),
    ],
    status: 'pending',
  };
